...
Page({
    ...
    formdata: function(e) {
        var that = this;
        // var fields = new Array(); //报名所需
        // //筛选报名所需字段
        // var activites = that.data.activities;
        // for (var i = 0; i < activites.length; i++) {
        //     if (activites[i].select) {
        //         act.push(activites[i].filed);
        //         actword.push(activites[i].name);
        //     }
        // }
        ...
        if (imgList.length == 0) {
            imgList[0] = "../../images/picturePoster-1.png";
        }
        ...
        if (flag1 && flag2 && flag3) {
            console.log(that.data.fields);
            wx.request({
                url: that.data.ip1 + "/activeWeb/createActive",
                data: {
                ...
                success: function(res) {
                    if (res.data != "error") {
                        ...
                        var count = 0; //第几张
                        var imgList = that.data.imgList;
                        if (length != 0) { that.uploadOneByOne(imgList, successUp, failUp, count, length); }
                        //that.UploadFile();
                    } else {
                        wx.showModal({
                            title: '提示',
        ...
        wx.uploadFile({
            ...
            complete: function(e) {
                ...
                if (count == length) {
                    ...
                    var major = that.data.major; //主题
                    var starttime = that.data.starttime; //开始时间
                    var endtime = that.data.endtime; //结束时间
                    var place = that.data.place; //地点

                    wx.redirectTo({
                        url: '../signUpSuccess/signUpSuccess?starttime=' + starttime + '&&endtime=' + endtime + '&&place=' + place + '&&major=' + major + "&&isCreate=1" + "&&pwdid=" + pwdid
                    })
    ...
    startbindMultiPickerChange: function(e) {
        ...
        this.setData({
                starttimeJudge: year + month + day + hour + minute,
    ...
    endbindMultiPickerChange: function(e) {
        ...
        // console.log(`${year}-${month}-${day}-${hour}-${minute}`);
        this.setData({
                endtimeJudge: year + month + day + hour + minute,
